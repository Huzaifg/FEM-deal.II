...
double FEM<dim>::basis_function(unsigned int node, double xi){
  ...
  //EDIT
  for(int b = 0;b<basisFunctionOrder+1;b++){
    if (b==node){
        continue;
    }
    else{
        value = ((xi - xi_at_node(b))/(xi_at_node(node)-xi_at_node(b)))*value;
    }
  }
  return value;
}
...
double FEM<dim>::basis_gradient(unsigned int node, double xi){
  ...
  //EDIT
  double value1=0;
  for(int l = 0;l<basisFunctionOrder+1;l++){
        if (l==node){
            continue;
        }
        else{
            value1 = (1/(xi_at_node(node)-xi_at_node(l)));
            for(int m = 0;m<basisFunctionOrder+1;m++){
                if (m==node||m==l){
                    continue;
                }
                else{
                    value1 = ((xi - xi_at_node(m))/(xi_at_node(node)-xi_at_node(m)))*value1;
                }
...
void FEM<dim>::assemble_system(){
  ...
  for (;elem!=endc; ++elem){
	...
	//EDIT - Define Flocal.
        Flocal[A]+= (h_e*pow(10,-4)/2)*basis_function(A,quad_points[q])*pow(10,11)*x*quad_weight[q];
      }
    }
    //Add nonzero Neumann condition, if applicable
    if(prob == 2){
      if(nodeLocation[local_dof_indices[1]] == 0.1){
	...
	for(unsigned int q=0; q<quadRule; q++){
	  //EDIT - Define Klocal.
	  Klocal[A][B]+=((2*pow(10,11)*pow(10,-4))/h_e)*basis_gradient(A,quad_points[q])*basis_gradient(B,quad_points[q])*quad_weight[q];
	}
      }
    }
	...
	double C= Klocal[i][j];
	K.add(local_dof_indices[i],local_dof_indices[j],C);
...
double FEM<dim>::l2norm_of_error(){
  ...
  for (;elem!=endc; ++elem){
	...
	u_h += D[local_dof_indices[B]]*basis_function(B,quad_points[q]);
      ...
      if(prob==1){
        l2norm+=pow(((x*((0.07/6.)-(pow(x,2.)/6.)))-u_h),2.)*quad_weight[q]*h_e/2.;
      }
      else{
        l2norm+=pow(((x*((0.105)-(pow(x,2.)/6.)))-u_h),2.)*quad_weight[q]*h_e/2.;
      }
