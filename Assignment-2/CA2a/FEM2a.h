...
std::vector<double> FEM<dim>::basis_gradient(unsigned int node, double xi_1, double xi_2){
  ...
  //EDIT
  if(node==0){
    values[0] = -0.5*(1-xi_2)/2;
    values[1] = -0.5*(1-xi_1)/2;
  }
  else if(node==1){
    values[0] = 0.5*(1-xi_2)/2 ;
    values[1] = -0.5*(1+xi_1)/2;
...
void FEM<dim>::define_boundary_conds(){
  ...
  for(unsigned int globalNode=0; globalNode<totalNodes; globalNode++){
    if(nodeLocation[globalNode][1] == 0){
      boundary_values[globalNode] = 300*(1+((0.3333)*nodeLocation[globalNode][0]));
    }
...
void FEM<dim>::assemble_system(){
  ...
  for (;elem!=endc; ++elem){
	  ...
	  for(unsigned int B=0; B<dofs_per_elem; B++){
		...
		for(unsigned int I=0;I<dim;I++){
		  for(unsigned int J=0;J<dim;J++){
		    ...
		    Klocal[A][B] += (basis_gradient(A,quad_points[q1],quad_points[q2])[i]*invJacob[i][I])*kappa[I][J]*(basis_gradient(B,quad_points[q1],quad_points[q2])[j]*invJacob[j][J]*detJ*quad_weight[q1]*quad_weight[q2]);
		  }
		}
	      }
	    }
	...
	double C= Klocal[A][B];
	K.add(local_dof_indices[A],local_dof_indices[B],C);
      }
    }
