...
void FEM<dim>::define_boundary_conds(){
  ...
  const unsigned int totalNodes = dof_handler.n_dofs(); //Total number of nodes
    ...
    if(nodeLocation[globalNode][0] == 0){
      boundary_values[globalNode] = 300*(1+(0.3333*(nodeLocation[globalNode][1]+nodeLocation[globalNode][2])));
    }
    if(nodeLocation[globalNode][0] == 0.04){
	boundary_values[globalNode] = 310*(1+(0.3333*(nodeLocation[globalNode][1]+nodeLocation[globalNode][2])));
      }
  }
}
